...
const Request = () => {
  ...
  const [states, setStates] = useState({
    ...
    driverList: [{id: "223546472525", password: "driv@!er", name: "yemi Craig"}, {id: "223546472524", password: "DRIV@!ER", name: "Abel Ola"}],
    driverID: "", 
    streamlinedList: [],
    ...
    displayRequestAlert: false,
    displayArrivalAlert: false,
    deliveryCode: "",
    deliveryError: true,
    deliverItem: []
  })
  ...
  const handleSubmitRequestTransfer = (e) => {
    ...
    item[0].source = driver.name
    const newList = removedItemList.concat(item)

      if (driver){
        setStates({...states, displayTransferError: false, displayRequestAlert: true, requestList: newList})
        console.log("Yaay!!!!")
      }
      else {
  ...
  const handleSucessfulTransferBox = () => {
    setStates({...states, displayRequestAlert: false, displayArrivalAlert: false, threeDotsOptions: "none"})
  }

  const handlePackageDelivery = () => {
    setStates({...states, deliveryRequest: "arrived", deliveryDetailView: "none", displayArrivalAlert: true})
  }
  ...
  const handleDeliveryCodeInputChange = (e) => {
    const {name, value} = e.target;

    const item = list.filter( item => item.deliveryCode === states.deliveryCode && item.id === states.trackingNum)

    const deliverItem = states.deliverItem
    const newDeliverItem = deliverItem.concat(item)

    console.log("item", item)
    if (item.length === 0){
      setStates({...states, [name]:value, deliveryError: true, deliverItem: [] })
    }
    else {
      setStates({...states, [name]:value, deliveryError: false, deliverItem: newDeliverItem })
    }
    
  }

  console.log(states)
  console.log("streamlined List ", states.streamlinedList )
...
{/* Delivered Items */}
      ...
      {states.deliveryRequest === "arrived" &&
        <DeliveryForm 
          ...
          inputChange={handleDeliveryCodeInputChange}
          deliveryCode={states.deliveryCode}
          error={states.deliveryError}
        />
      }


      {states.displayRequestAlert && 
        <Alert
          bold="Request was sent successfully"
          lineOne="We are notifying the Driver"
          lineTwo="about your request..."
          click={handleSucessfulTransferBox}
        />
      }

      {states.displayArrivalAlert && 
        <Alert
          bold="Well done how was the journey?"
          lineOne="We are sending a notification to the"
          lineTwo="Receiver that you have arrived..."
          click={handleSucessfulTransferBox}
        />
      }
